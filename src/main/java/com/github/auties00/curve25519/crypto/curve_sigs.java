...
public class curve_sigs {
    ...
    public static int curve25519_sign(byte[] signature_out,
        ...
        sign_modified.crypto_sign_modified(sigbuf, msg, msg_off, msg_len, curve25519_privkey, ed_pubkey, random);
    ...
    public static int curve25519_verify(byte[] signature, int signature_off,
                                        byte[] curve25519_pubkey,
                                        int curve25519_pubkey_off,
                                        byte[] msg, int msg_off, int msg_len) {
        int[] mont_x = new int[10];
      ...
      */
        fe_frombytes.fe_frombytes(mont_x, curve25519_pubkey, curve25519_pubkey_off);
        fe_1.fe_1(one);
        ...
        ed_pubkey[31] |= (signature[signature_off + 63] & 0x80);
        System.arraycopy(signature, signature_off, verifybuf, 0, 64);
        verifybuf[63] &= 0x7F;

        System.arraycopy(msg, msg_off, verifybuf, 64, msg_len);
