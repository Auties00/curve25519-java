    ...
    */
    public static int crypto_sign_modified(
        ...
        if(random != null) {
            ...
            System.arraycopy(message, messageOffset, out, 64, messageLength);

            /* NEW: add suffix of random data */
            System.arraycopy(random, 0, out, messageLength + 64, 64);

            Sha512.calculateDigest(r, out, 0, messageLength + 128);
        ...
        Sha512.calculateDigest(r, out, 32, out.length - 32);
        return crypto_sign_modified(out, messageLength, privateKey, publicKey, h, r, p);
    }

    private static int crypto_sign_modified(byte[] sm, int n, byte[] sk, byte[] pk, byte[] h, byte[] r, ge_p3 p) {
        ...
        Sha512.calculateDigest(h, sm, 0, n + 64);
